<x-layouts.app title="Activity Feed">
    <div class="space-y-6">
            ...
            <section class="bg-surface-container-low rounded-xl overflow-hidden border-l-4 border-stage-stuck shadow-[0_0_40px_-16px_rgba(239,159,39,0.3)]">
                ...
                <div class="divide-y divide-outline-variant/20">
                    @foreach ($stuckRuns as $run)
                        <a href="{{ route('issues.show', $run->issue) }}" class="flex items-center justify-between gap-3 px-4 py-2 hover:bg-surface-container transition-colors">
                            <div class="min-w-0">
                                <div class="flex items-center gap-2">
        ...
        {{-- Filter pills row --}}
        <form method="GET" action="{{ route('activity.index') }}" class="bg-surface-container-low rounded-xl p-4">
            <div class="grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
                <div>
                    <label for="source" class="block font-label text-[10px] text-outline uppercase tracking-widest mb-1">Source</label>
                    <select name="source" id="source" class="w-full rounded-md bg-surface-container-lowest border-outline-variant text-on-surface text-sm px-3 py-1.5">
                ...
                <div>
                    ...
                    <select name="stage" id="stage" class="w-full rounded-md bg-surface-container-lowest border-outline-variant text-on-surface text-sm px-3 py-1.5">
                ...
                <div>
                    ...
                    <select name="actor" id="actor" class="w-full rounded-md bg-surface-container-lowest border-outline-variant text-on-surface text-sm px-3 py-1.5">
                ...
                <div>
                    ...
                    <select name="autonomy" id="autonomy" class="w-full rounded-md bg-surface-container-lowest border-outline-variant text-on-surface text-sm px-3 py-1.5">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="rounded-md bg-primary text-on-primary px-3 py-1.5 font-label text-[10px] uppercase tracking-widest hover:bg-primary/90">
                        Filter
                    </button>
                    @if (request()->hasAny(['source', 'stage', 'actor', 'autonomy']))
                        <a href="{{ route('activity.index') }}" class="rounded-md bg-surface-container-high text-on-surface-variant px-3 py-1.5 font-label text-[10px] uppercase tracking-widest hover:bg-surface-container-highest">
                            Clear
                        </a>
                    @endif
                </div>
        ...
        @else
            <div class="space-y-3">
                @foreach ($events as $event)
                    ...
                    @endphp
                    <div class="rounded-xl {{ $cardClass }} px-4 py-3">
