...
class extends Component {
    ...
    public function with(): array
    {
        ...
        return [
            'iterations' => $this->groupByIteration($this->run),
            'prUrl' => $this->findPrUrl($this->run),
        ];
    }
...
<div class="space-y-4">
    @foreach ($iterations as $iterationNum => $stages)
        ...
        @endphp
        <details {{ $isLatest ? 'open' : '' }} class="group" wire:key="iter-{{ $iterationNum }}">
            ...
            @else
                <div class="space-y-4">
                    @foreach ($stages as $stage)
                        <div class="rounded-xl bg-surface-container-low overflow-hidden" wire:key="stage-{{ $stage->id }}">
